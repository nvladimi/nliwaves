...
def Test():     #fbase,  fbaseout=''):
    ...
    pulse2D(f, j, dj=0, ntime=200, n=10000, frange=(1000,1010), fbaseout='testout/'+f+'_p3test');
...
def pulse2D(fbase, j, dj=0, ntime=1000, n=10000, frange=(0,10), convert=False, fbaseout=''):
    ...
    j = np.array(j)
    Q  = np.zeros((ntime, len(j)), dtype=np.complex128)
    R  = np.zeros((ntime, len(j)), dtype=np.float64)
    ntot = 0
    
    for s in sets:

        A1 = np.zeros(( (frange[1]-frange[0])*n, len(j)),  dtype=np.complex64)
        A2 = np.zeros(( (frange[1]-frange[0])*n, len(j)),  dtype=np.complex64)
        ...
        for n0 in np.arange(ntime):

            a1 = A1[ 0: -ntime,    :]
            a2 = A2[n0: -ntime+n0, :]

            q = a1 * np.conjugate(a2)
            r = np.sqrt(np.real(q * np.conjugate(q)))
    ...
    Q.tofile( fbaseout + "Q{}.dat".format(dj) )
    R.tofile( fbaseout + "R{}.dat".format(dj) )
