...
x=np.arange(0,21)
iax.plot(x,-x, ':k', lw=LW, label="$\\rm Gauss$")

   
iax.set_xlim(0,18)
iax.set_ylim(-18,0)
iax.set(xlabel='$|b_j|^2 / \langle |b_j|^2 \\rangle $')
iax.set_yticks(np.arange(-18, 1, step=6)) 
iax.yaxis.set_label_coords(-0.10,0.5)
iax.xaxis.set_label_coords(0.5,-0.11)

iax.text(1.3, -9, "$|j-d|$", fontsize=12)
...
inax.plot(x,1.13*x, ":k") # label="$1.13|j-d|$")
...
inax.text(95, 20, "$1.13|j-d|$", fontsize=11)
