...
outfile = "plot_corr_alpha18_summary.pdf"
#outfmt = "{:>8.4f}{:>8.4f}"
#lblfmt = "{:>6.3f} i {:>+5.2f}"
...
for run in runs:
    ...
    for c in corrs:
        ...
        nk = dat[:,1]
        x = dat[:,0]
        y = dat[:,3]/dat[:,2] / ffactor[c]

        yavg = np.average(y[20:(nmodes-20)])
        yrms = np.sqrt(np.average( (y[20:(nmodes-20)] - yavg)**2 ) )
...
dat = np.loadtxt("corr_alpha18_all.txt")
...
#iax.plot(i, 10*np.exp(-i), color="k", lw=LW/2)
iax.plot(i, 1.4*i**(-3.5), color="k", lw=LW/2, label="$\propto m^{-3.5}$")
