...
for j in range(0,4):
    for i in range(0,3):
        ...
        ax[j][i].legend(frameon=False, handlelength=1)
    ax[j][0].set(xlabel='$\ln|i-d|$', ylabel="$\ln|C_{" + corr[j][0]  +"}|$")
    ax[j][1].set(xlabel='$i$', ylabel="$C_{" + corr[j][0]  +"}$")
    ax[j][2].set(xlabel='$i$', ylabel="$C_{" + corr[j][0]  +"}$")
    ax[j][0].set_xlim(0,5.5)
