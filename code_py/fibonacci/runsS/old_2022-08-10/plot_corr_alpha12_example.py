...
outfile = "plot_corr_alpha12_example.pdf"
#outfmt = "{:>8.4f}{:>8.4f}"
#lblfmt = "{:>6.3f} i {:>+5.2f}"
...
labels = ("$D_{21\\bar{0}}$", "$D_{322\\bar{0}}$", "$D_{431\\bar{0}}$", "$D_{\\bar{3}11\\bar{0}}$")
...
fig, ax = plt.subplots(ncols=2, figsize=(6.5,2.5))
...
labels = ("$D_{2211\\bar{0}\\bar{0} }$",
          "$D_{3221\\bar{1}\\bar{0}}$",
          "$D_{\\bar{3}\\bar{2}211\\bar{0}}$",
          "$D_{432\\bar{2}1\\bar{0}}$",
          "$D_{\\bar{4}\\bar{3}211\\bar{0}}$")
...
iax.set(xlabel='$|i - d|$') # ylabel="$D_{" + corr[j][0]  +"} \,  n^{1 - m/2}$")
iax.set_xlim(0,nmodes)
iax.legend(frameon=False, handlelength=0.1, loc="upper left", bbox_to_anchor=(-0.50,1.05))
iax.set_yticks(np.arange(-0.6, 0.7, step=0.2))
...
plt.subplots_adjust(left=0.15, right=0.98, top=0.97, bottom=0.18, hspace=0.15, wspace=0.55)
