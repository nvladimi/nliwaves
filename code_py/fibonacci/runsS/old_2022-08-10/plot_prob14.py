...
outfile = "plot_prob14.pdf"

#-- common parameters --

prefix = "MIx32x1/"

fbasedir = ["s3v34", "s4v34"]
fbaseinv = ["s3v14", "s4v14"]
...
ax[0][0].text(9,-2, "$\\alpha = 3/4$ (direct)")
ax[1][0].text(9,-2, "$\\alpha = 1/4$ (inverse)")
...
for iax in (ax[0][1], ax[1][1]):  
    ...
    iax.set_yticks(np.arange(-0, 11, step=2))
    iax.yaxis.labelpad = 0
    #iax.legend(frameon=False)

for iax in (ax[0][2], ax[1][2]):  
    iax.set_xlim(-1,1)
    iax.set_ylim(-0.06,0.06)
    iax.set(xlabel='$\\theta_i / \pi$', ylabel='${\cal P}  - 1/2$')
    iax.set_yticks(np.arange(-0.06, 0.061, step=0.02))
