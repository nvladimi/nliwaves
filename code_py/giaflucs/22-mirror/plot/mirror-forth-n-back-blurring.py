...
outfile = "mirror-forth-n-back-blurring.pdf"
...
for rset in runs:
    run = rset[0]
    fname = "../2022-05-04_blurring/data/lega" + run + "/lega" + run + ".000000" 
    print(fname)
...
for rset in runs:
    run = rset[0]
    fname = "../2022-05-04_blurring/data/zero" + run + "/zero" + run + ".000000" 
    print(fname)
    label = rset[2]
    ...
    ax[2][1].plot(x,y3, '-o',  mfc='none', ms=MS, lw=LW) #, style, mfc='none', ms=MS,  lw=LW,  label=label);
...
ax[0][0].text(1.5, 1.56, "legacy parameters, atmosphere", fontsize=12)
ax[0][1].text(2.5, 1.04, "legacy parameters, vacuum",  fontsize=12)
...
ax[3][0].set_ylim(0,90)
ax[3][1].set_ylim(0,30)
...
plt.subplots_adjust(left=0.08, right=0.98, top=0.94, bottom=0.08, wspace=0.25, hspace=0.25)
